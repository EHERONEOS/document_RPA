...
| 浏览器日清理 | 未描述 | 每天 `20:30` 关闭空闲/孤儿 RPA 浏览器，忙碌任务等待或跳过，主 profile 保留 | 避免长驻 Chromium 进程异常，同时不误杀正在执行的任务。 |
...
| 队列恢复 | 已描述心跳后 `sync` 快照恢复 | 当前实现仍每 15 秒心跳、启动即心跳，并在有效心跳后下发完整分配快照 | `server` 模式仍以中心分配为准；控制面失联时按 `RPA_QUEUES` 兜底，恢复后仍走 `sync`。 |
