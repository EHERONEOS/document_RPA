...
- 同一设备可同时运行多个队列；默认由中心分配决定数量。`QUEUE_ASSIGNMENT_SOURCE=local` 时按 `RPA_QUEUES` 启动；`server` 模式下 `RPA_QUEUES` 仅作控制面失联兜底。
...
5. 每天 `BROWSER_CLEANUP_TIME`（默认 20:30）会关闭空闲和孤儿 RPA 浏览器进程。主 profile 目录仍保留，下次任务重新启动浏览器；清理时会挡住新的槽位申请，并等待忙碌任务最多 `BROWSER_CLEANUP_WAIT_BUSY_SECONDS`。
