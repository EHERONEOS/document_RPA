...
队列由独立控制平台绑定到具体设备。设备侧 Agent 收到 Redis Streams 命令后，才为该设备
负责的队列创建独立 Worker；业务代码仍要求“船司目录存在”且该船司 `router.py` 有完整路由。
...
| 队列控制 | MySQL、Redis Streams、FastAPI | 保存设备/队列状态，通过可靠命令流控制机器侧队列控制客户端。 |
...
.
├── app
│   ├── main.py                    # 设备侧 Queue Agent 入口
│   ├── control/                   # 本机 Worker 监管和安全排空
...
│   └── dev/                       # 本地 JSON 消息调试
├── queue_control_platform/        # 独立部署的队列控制平台
│   ├── server/                    # MySQL 仓储、Redis 事件处理与 API
│   ├── agent/                     # 设备身份、Redis 命令循环
│   ├── frontend/                  # 独立维护页面
│   └── docker/                    # 本地 MySQL 和 Redis Compose 定义
├── doc/                           # 项目文档
...
| 模式 | 入口 | 行为 |
| --- | --- | --- |
| 设备队列控制客户端 | `python -m app.main` | 使用设备 ID 消费 Redis 命令，为已分配队列创建独立 Worker。 |
| FastAPI 维护平台 | `python -m queue_control_platform.server.main` | 通过 Uvicorn 提供维护页面，在 FastAPI 生命周期中消费客户端事件并将状态写入 MySQL。 |
...
2. 仅在 `NACOS_ENABLED=true` 时调用 `load_nacos_environment()` 加载通用远端配置。
3. `Settings.from_env()` 读取浏览器等本地运行参数。
4. RabbitMQ 配置默认由 `RabbitmqSettings` 在实际创建消费者时，从 Nacos 的
   `rabbitmq.yml` 按 `APP_ENV` 单独读取。

因此，设备 Agent 启动不会因导入 `app.config` 自动请求 Nacos；实际分配队列并创建
RabbitMQ 消费者时，才会按 RabbitMQ 配置源读取连接信息。

### 4.3 当前生效的配置项

| 配置来源 | 关键项 | 使用位置 |
| --- | --- | --- |
| `.env` | `QUEUE_CONTROL_DEVICE_ID`、`QUEUE_CONTROL_DEVICE_TOKEN` | 设备侧 Agent 的平台注册身份。 |
| `.env` | `QUEUE_CONTROL_REDIS_URL` | Agent 命令和状态事件的 Redis Streams 地址。 |
| `.env` | `QUEUE_CONTROL_MYSQL_URL` | 中心维护平台写入队列状态和命令记录的 MySQL 地址。 |
...
RabbitMQ Nacos 配置兼容 `NACOS_SERVER_ADDRESSES`、`NACOS_NAMESPACE` 以及旧版
`nacos_ip`、`nacos_namespace` 环境变量。

## 5. 队列消息与路由

### 5.1 队列命名

输入队列名称由统一入口在各船司目录的 `router.py` 中按完整值查找，不依赖名称中 `_`
的数量或位置。路由项明确给出 `customer_code` 和 `business_code`；统一入口从目录名确定
`carrier_code`，再调用对应船司的路由文件。
...
| 能力 | 当前状态 |
| --- | --- |
| RabbitMQ 多队列消费 | 已实现，由控制平台中的设备-队列绑定控制。 |
| 设备队列维护 | 已实现，维护页面可创建设备、绑定队列、暂停、恢复和重启。 |
| 状态和控制命令持久化 | 已实现，状态写入 MySQL，命令和事件通过 Redis Streams 可靠传递。 |
