...
| 设备 Queue Agent | `uv run python -m app.main` | 按开关接收 Redis 命令或读取本地 `RPA_QUEUES`，创建本机账号协调器、监管队列 Worker。 |
...
默认 `QUEUE_ASSIGNMENT_SOURCE=server` 时，队列范围以中心 `queue_assignments` 为准；Worker 只有在收到 `assign` 或 `sync` 后才会启动。控制面 Redis 不可达时，Agent 保持进程存活，并按 `RPA_QUEUES` 补齐兜底监听；连接恢复后仍以平台心跳触发的 `sync` 全量对账。`QUEUE_ASSIGNMENT_SOURCE=local` 时不连接控制平台，启动即按 `RPA_QUEUES` 监听。`app.queue.consumer.start_consumers()` 仍保留为兼容入口，但正式设备入口不使用它。
...
| 配置 | 作用 | 默认或约束 |
| --- | --- | --- |
| `QUEUE_CONTROL_DEVICE_ID` / `QUEUE_CONTROL_DEVICE_TOKEN` | 设备身份和平台注册令牌 | `server` 模式必填；`local` 模式可不填。 |
| `QUEUE_ASSIGNMENT_SOURCE` | 队列监听来源 | `server`（默认，中心下发）或 `local`（本地 `RPA_QUEUES`）。 |
...
| `BROWSER_CLEANUP_ENABLED` | 是否启用浏览器日清理 | 默认启用。 |
| `BROWSER_CLEANUP_TIME` | 每天关闭空闲 RPA 浏览器的本地时刻 | 默认 `20:30`。 |
| `BROWSER_CLEANUP_WAIT_BUSY_SECONDS` | 日清理等待忙碌槽位结束的最长时间 | 默认 1800 秒。 |
...
`QueueControlClient` 启动一个 `QueueSupervisor` 与一个 `multiprocessing.Manager` 承载的 `AccountSessionCoordinator`。`server` 模式下 Agent 将命令映射为 Supervisor 操作，并在操作成功或失败后回传 `command_result`；无论操作结果如何，当前实现都会确认已读取的命令，以避免错误命令永久阻塞命令流。控制面 Redis 连不上时不会退出进程，而是按 `RPA_QUEUES` 启用本地兜底；恢复后等待平台 `sync` 回到服务端权威列表。
...
5. 每天 `BROWSER_CLEANUP_TIME`（默认 20:30）关闭空闲及孤儿 RPA 浏览器进程，避免长驻 Chromium 异常；清理期间新任务会等待槽位，忙碌任务最多等待 `BROWSER_CLEANUP_WAIT_BUSY_SECONDS`。主 profile 目录保留，下次任务重新拉起浏览器。
...
业务任务在 `handle_message()` 的 `finally` 中释放账号槽位；Worker 异常退出或被强制终止时，Supervisor 会释放该进程名下遗留的租约。临时 profile 在账号空闲超过 `ACCOUNT_IDLE_SECONDS` 后关闭浏览器、释放端口并删除目录，主 profile 会保留以复用会话。每天到点后，协调器会等待忙碌槽位结束，再终止空闲/孤儿 RPA 浏览器；正在执行的任务不会被强制杀掉，超时仍忙碌的进程会跳过并记警告。
