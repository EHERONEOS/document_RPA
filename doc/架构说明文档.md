...
| 队列控制 | MySQL、Redis Streams、Flask | 保存设备/队列状态，通过可靠命令流控制机器侧 Agent。 |
...
| 模式 | 入口 | 行为 |
| --- | --- | --- |
| 设备 Queue Agent | `python -m app.main` | 使用设备 ID 消费 Redis 命令，为已分配队列创建独立 Worker。 |
| 维护平台 | `python -m queue_control_platform.server.main` | 提供维护页面，消费 Agent 事件并将状态写入 MySQL。 |
