...
class RedisStreamBus:
    ...
    # 将控制命令写入目标设备专属的 Redis Stream。
    def send_command(self, device_id: str, command: dict[str, Any]) -> str:
        return self._client().xadd(
            self.command_stream(device_id), {"payload": json.dumps(command, ensure_ascii=False)}
        )

    # 将 Agent 状态、心跳或命令结果写入中心事件流。
    ...
    # 将 Redis 返回的嵌套消息结构转换为事件 ID 与 JSON 载荷列表。
    @staticmethod
